...
function createStringandSort(string) {
    return string.replace(/ /g, '').toLowerCase().split('').sort().join('');
}

function isAnagramInArray(anagram, arr){
    let toCompare = createStringandSort(anagram)
    const anagramArray = arr.filter((arrItem) => {
        let toCompareWith = createStringandSort(arrItem)
